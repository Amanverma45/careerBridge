...
export const validateText = (text, fieldName) => {
  ...
  // We allow digit repetition (e.g. "10000" or "9999" is fine).
  ...
  // This catches random consonant sequences like "tkhgfkfh", "dfdfdf", "klffjh", "ggggg".
  const words = trimmed.toLowerCase().split(/[\s,./()\-+]+/);
  for (let word of words) {
    // Only check words containing alphabetical characters
...
export const validateUrl = (url, fieldName) => {
  ...
  try {
    // If it doesn't start with http/https, try parsing with https prefix
