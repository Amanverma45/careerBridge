          ...
          {token && (
            <>
              <Link
                to={getDashboardLink()}
                className={`relative px-3 py-2 text-sm font-semibold tracking-wide transition-all duration-300 group ${
                  isActive(getDashboardLink()) ? 'text-brand-secondary' : 'text-slate-655 hover:text-brand-secondary dark:text-slate-300 dark:hover:text-brand-secondary'
                }`}
              >
                Dashboard
                <span className={`absolute bottom-0 left-0 h-[2px] bg-brand-secondary transition-all duration-300 ${
                  isActive(getDashboardLink()) ? 'w-full' : 'w-0 group-hover:w-full'
                }`} />
              </Link>
